...
void insertion_sort_deck_kind(deck_node_t **deck)
{
	deck_node_t *first_iteration, *insert_val, *val_tmp;

	for (first_iteration = (*deck)->next; first_iteration != NULL; first_iteration = val_tmp)
	{
		val_tmp = first_iteration->next;
...
void insertion_sort_deck_value(deck_node_t **deck)
{
	...
	for (first_iteration = (*deck)->next; first_iteration != NULL; first_iteration = val_tmp)
	{
		val_tmp = first_iteration->next;
		insert_val = first_iteration->prev;
		while (insert_val != NULL &&
		       insert_val->card->kind == first_iteration->card->kind &&
		       get_value(insert_val) > get_value(first_iteration))
		{
			insert_val->next = first_iteration->next;
			if (first_iteration->next != NULL)
				first_iteration->next->prev = insert_val;
